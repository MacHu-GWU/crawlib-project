...
def create_cache_here(this_file,
                      compress_level=6,
                      value_type_is_binary=False,
                      **kwargs):  # pragma: no cover
    return create_cache(
        directory=os.path.join(os.path.dirname(this_file), ".cache"),
        compress_level=compress_level, value_type_is_binary=value_type_is_binary,
        **kwargs
    )



def log_cache_does_not_hit(cache, key):
    if key not in cache:
        msg = "{} doesn't hit cache!".format(key)
        print(msg)
...
class CacheBackedDownloader(object):
    ...
    :type cache_value_type_is_binary: bool
    :param cache_value_type_is_binary: bool

    :type cache_compress_level: int
